...
const Popup = () => {
    ...
    const multipliedResult = betData.reduce((accumulator, currentValue) => {
        return accumulator * currentValue.cellData;
    }, 1);
            ...
            <div className="popup-content">
                {betData?.map((bet, index) =>
                    <div className='popup-row' key={`${bet?.data?.C}${index}`}>
                        <div>{bet?.data?.OCG?.[1]?.MBS} Kod: {bet?.data?.C}</div>
